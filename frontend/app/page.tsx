...
import {
  ...
  PieChart, ArrowRight, DollarSign, ShoppingCart, Users, TrendingDown
} from "lucide-react"
...
export default function DashboardPage() {
  ...
  const merchantSalesShare = merchantData
    ? merchantData.map((m: any, index: number) => {
        const totalAmount = m.total_amount || m.total_revenue_rial || 0
        return {
          ...m,
          rank: index + 1,
  ...
  return (
    <div className="container mx-auto py-4 space-y-8 px-4 md:px-6">
      ...
      {/* Performance Metrics */}
      <section>
        <h2 className="text-lg font-semibold mb-4 text-foreground">شاخص‌های کلان</h2>
        {overviewLoading ? (
      ...
      {/* Nowruz Analytics - prominently featured */}
      <section>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-foreground flex items-center gap-2">
      ...
      {/* Two-column layout: Category Distribution + Status Breakdown */}
      ...
      {/* Two-column layout: Merchant Ranking (clickable) + Peer Comparison */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <section>
          <h2 className="text-lg font-semibold mb-4 text-foreground flex items-center gap-2">
            <BarChart3 className="h-5 w-5 text-primary" />
        ...
        <section>
          <h2 className="text-lg font-semibold mb-4 text-foreground">مقایسه با همتایان</h2>
          <Suspense fallback={<Skeleton className="h-80 w-full" />}>
            <PeerComparison
      ...
      {/* Recommendations */}
      <section>
        <h2 className="text-lg font-semibold mb-4 text-foreground">پیشنهادات هوشمند</h2>
        <Suspense fallback={<Skeleton className="h-80 w-full" />}>
...
// Merchant Ranking Table component
function MerchantRankingTable({ merchants, onSelect }: {
  merchants: any[]
  onSelect: (merchantKey: string) => void
}) {
  ...
  return (
    <Card className="bg-card/50 border-border/50">
      <CardHeader>
        <CardTitle>جدول رتبه‌بندی فروشگاه‌ها</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {merchants.map((m, index) => (
                <tr key={m.merchant_key} className="border-b hover:bg-muted/30">
                  ...
                  <td className="py-2">{m.category_title || "—"}</td>
                  <td className="py-2">{toPersianNumber(m.total_attempts || 0)}</td>
                  <td className="py-2">{formatCurrencyIRToman(m.total_amount || 0)}</td>
                  <td className="py-2">{toPersianNumber((m.success_rate_pct || 0).toFixed(1))}%</td>
                  <td className="py-2 text-center">
                    <Button
                      size="sm"
